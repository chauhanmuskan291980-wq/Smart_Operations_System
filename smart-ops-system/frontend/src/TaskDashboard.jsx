...
import { Plus, Trash2, CheckCircle2, Circle, User, LayoutDashboard, ListTodo, X, Pencil } from 'lucide-react';
      ...
      <div className="flex-1 p-6 md:p-12 overflow-y-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Manage, track, and organize your team's workflow.</p>
          </div>
          ...
          {['ALL', 'PENDING', 'COMPLETED'].map(f => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all ${
                filter === f
                  ? 'bg-[#0a2b45] text-white shadow-md'
        ...
        {filteredTasks.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredTasks.map(task => (
              ...
              >
                {/* Top Accent Line */}
                <div className={`absolute top-0 left-0 w-full h-1 ${task.status === 'COMPLETED' ? 'bg-emerald-500' : 'bg-[#0a2b45]'}`}></div>

                <div className="flex justify-between items-start mb-3">
                  <h3 className="font-bold text-lg text-slate-800 pr-4 leading-tight">{task.title}</h3>
                  {isAdmin && (
