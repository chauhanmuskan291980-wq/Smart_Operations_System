...
export default function TaskDashboard() {
  ...
  // Fetch tasks for both the Dashboard and Tasks tabs
  useEffect(() => {
          ...
          <nav className="space-y-2">
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-colors ${
                activeTab === "DASHBOARD" ? "bg-[#0a2b45]/5 text-[#0a2b45]" : "text-slate-500 hover:bg-slate-50"
              }`}
            >
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-[#0a2b45] rounded-3xl p-8 mb-8 text-white shadow-lg relative overflow-hidden">
              <div className="relative z-10">
                ...
                <p className="text-slate-300 max-w-lg leading-relaxed">
                  Here is what's happening with your projects today. You have {pendingTasks} pending tasks that need your attention.
                </p>
                <button 
                  onClick={() => setActiveTab("TASKS")}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
                ...
                <div>
                  <h3 className="text-3xl font-bold text-slate-800">{inProgressTasks}</h3>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                {tasks.length === 0 && (
                  <p className="text-slate-500 text-sm text-center py-4">No tasks found. Get started by creating one!</p>
                )}
              </div>
            </div>
          </div>
              ...
              {["ALL", "OPEN", "IN_PROGRESS", "COMPLETED"].map((f) => (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all whitespace-nowrap ${
                    filter === f ? "bg-[#0a2b45] text-white" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"
                  }`}
                >
                ...
                {filteredTasks.map((task) => (
                  <div key={task.id} className="group bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-xl transition-all relative flex flex-col">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <h3 className="font-bold text-lg text-slate-800 pr-4">{task.title}</h3>
                      {(isAdmin || isManager) && (
          ...
          <div className="bg-white p-8 rounded-3xl w-full max-w-md shadow-2xl relative animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input required value={form.title} className="w-full border border-slate-300 px-4 py-3 rounded-xl focus:ring-2 focus:ring-[#0a2b45] outline-none transition-shadow" onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Update user schemas" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">Description</label>
                <textarea rows="4" value={form.description} className="w-full border border-slate-300 px-4 py-3 rounded-xl focus:ring-2 focus:ring-[#0a2b45] outline-none transition-shadow resize-none" onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Details about the task..." />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">Assign To (User ID)</label>
                <input value={form.assignedToId} className="w-full border border-slate-300 px-4 py-3 rounded-xl focus:ring-2 focus:ring-[#0a2b45] outline-none transition-shadow" onChange={(e) => setForm({ ...form, assignedToId: e.target.value })} placeholder="Leave blank to unassign" />
              </div>
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setOpen(false)} className="flex-1 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-semibold transition-colors">Cancel</button>
                <button type="submit" className="flex-1 bg-[#0a2b45] hover:bg-[#082236] text-white rounded-xl font-semibold shadow-md transition-colors">Save Task</button>
              </div>
            </form>
          </div>
        </div>
